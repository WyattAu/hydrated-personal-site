...
fn rk4_step(dt: f64) {
    unsafe {
        ...
        let (k2v1, k2v2) = derivatives(
            t1 + k1v1.dtheta * dt * 0.5,
            t2 + k1v2.dtheta * dt * 0.5,
            w1 + k1v1.domega * dt * 0.5,
            w2 + k1v2.domega * dt * 0.5,
        );
        let (k3v1, k3v2) = derivatives(
            t1 + k2v1.dtheta * dt * 0.5,
            t2 + k2v2.dtheta * dt * 0.5,
            w1 + k2v1.domega * dt * 0.5,
            w2 + k2v2.domega * dt * 0.5,
        );
        let (k4v1, k4v2) = derivatives(
            t1 + k3v1.dtheta * dt,
            t2 + k3v2.dtheta * dt,
            w1 + k3v1.domega * dt,
            w2 + k3v2.domega * dt,
...
pub fn update_double_pendulum(canvas_id: &str, w: u32, h: u32, _t: f64) -> Result<(), JsValue> {
    ...
    unsafe {
        ...
        // Update trail
        if STATE.trail_len < STATE.trail.len() {
            STATE.trail[STATE.trail_len] = (x2, y2);
            STATE.trail_len += 1;
        } else {
            // Shift trail left
            for i in 0..(STATE.trail.len() - 1) {
                STATE.trail[i] = STATE.trail[i + 1];
            }
            STATE.trail[STATE.trail.len() - 1] = (x2, y2);
        }
        ...
        // Draw trail
        for i in 1..STATE.trail_len {
            let alpha = (i as f64 / STATE.trail_len as f64) * 0.6;
            let hue = 180.0 + (i as f64 / STATE.trail_len as f64) * 120.0;
        ...
        ctx.begin_path();
        ctx.arc(cx, cy, 3.0, 0.0, std::f64::consts::TAU).ok();
        ctx.fill();

        // Draw bob 1
        ctx.set_fill_style(&"#00e5ff".into());
        ctx.begin_path();
        ctx.arc(x1, y1, 8.0, 0.0, std::f64::consts::TAU).ok();
        ctx.fill();

        // Draw bob 2
        ctx.set_fill_style(&"#ff4081".into());
        ctx.begin_path();
        ctx.arc(x2, y2, 10.0, 0.0, std::f64::consts::TAU).ok();
        ctx.fill();
    }
